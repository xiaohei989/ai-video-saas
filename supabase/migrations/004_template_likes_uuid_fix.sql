-- ============================================
-- Template Likes UUID Fix Migration
-- Version: 004
-- Description: 适应现有UUID类型的templates表，创建点赞功能
-- ============================================
...
-- ============================================
-- 3. 创建模板点赞表（使用UUID匹配现有templates表）
-- ============================================
...
-- ============================================
-- 4. 创建模板收藏表（使用UUID匹配现有templates表）
-- ============================================
...
-- ============================================
-- 9. 插入示例模板数据（转换为UUID格式）
-- ============================================
DO $$
BEGIN
  -- 检查 templates 表是否为空
  IF NOT EXISTS (SELECT 1 FROM public.templates LIMIT 1) THEN
    -- 插入示例模板数据（使用UUID，包含所有必需字段）
    INSERT INTO public.templates (id, name, slug, description, prompt_template, parameters, category, credit_cost, is_public, like_count) VALUES
    (gen_random_uuid(), '艺术咖啡机魔法创作', 'art-coffee-machine', 
     '将经典艺术作品或自定义图片通过神奇咖啡机转化为杯中的流体艺术',
     '将经典艺术作品或自定义图片通过神奇咖啡机转化为杯中的流体艺术 {image}',
     '[{"type":"image","name":"image","label":"Image","placeholder":"Click to upload an image","required":true,"accept":"image/*"}]'::jsonb,
     'asmr', 10, true, 520),
    (gen_random_uuid(), 'ASMR超现实吐司涂抹', 'asmr-surreal-toast-spread',
     '将各种意想不到的微型物品当作黄油涂抹在吐司上，创造视觉冲击的ASMR体验',
     '将各种意想不到的微型物品当作黄油涂抹在吐司上，创造视觉冲击的ASMR体验 {object}',
     '[{"type":"text","name":"object","label":"Object to spread","placeholder":"e.g., miniature toys, fruits","required":true}]'::jsonb,
     'asmr', 8, true, 342),
    (gen_random_uuid(), '玻璃水果切割ASMR', 'glass-cutting-asmr',
     'Create mesmerizing ASMR videos of cutting translucent glass fruits',
     'Create mesmerizing ASMR videos of cutting translucent glass {fruit}',
     '[{"type":"select","name":"fruit","label":"Fruit Type","required":true,"options":[{"value":"apple","label":"Apple"},{"value":"orange","label":"Orange"},{"value":"grape","label":"Grape"}],"default":"apple"}]'::jsonb,
     'asmr', 6, true, 489),
    (gen_random_uuid(), '魔法画笔3D绽放', 'magic-pen-3d-bloom',
     '用魔法画笔在书页上触发3D立体绽放效果的ASMR视频',
     '用魔法画笔在书页上触发3D立体绽放效果的ASMR视频 {effect}',
     '[{"type":"text","name":"effect","label":"Bloom Effect","placeholder":"e.g., flowers, butterflies, stars","required":true}]'::jsonb,
     'asmr', 12, true, 678),
    (gen_random_uuid(), '微型动物惊喜', 'miniature-animals-surprise',
     '超现实的微型动物藏在意想不到的容器中，开门瞬间的惊喜揭露',
     '超现实的微型{animal}藏在意想不到的{container}中，开门瞬间的惊喜揭露',
     '[{"type":"select","name":"animal","label":"Animal","required":true,"options":[{"value":"cat","label":"Cat"},{"value":"dog","label":"Dog"},{"value":"rabbit","label":"Rabbit"}],"default":"cat"},{"type":"text","name":"container","label":"Container","placeholder":"e.g., box, drawer, jar","required":true}]'::jsonb,
     'fun', 10, true, 756),
    (gen_random_uuid(), '监控动物奇遇', 'surveillance-animal-encounter',
     '创建独特的监控摄像头视角动物互动视频，带有夜视效果和鱼眼镜头畸变',
     '创建独特的监控摄像头视角{animal}互动视频，带有夜视效果和鱼眼镜头畸变',
     '[{"type":"select","name":"animal","label":"Animal","required":true,"options":[{"value":"cat","label":"Cat"},{"value":"dog","label":"Dog"},{"value":"bird","label":"Bird"}],"default":"cat"}]'::jsonb,
     'fun', 8, true, 423),
    (gen_random_uuid(), '指尖萌宠', 'tiny-pet-fingertip',
     '拍摄一只站在指尖上的超迷你可爱动物，展现温馨治愈的画面',
     '拍摄一只站在指尖上的超迷你可爱{pet}，展现温馨治愈的画面',
     '[{"type":"select","name":"pet","label":"Pet Type","required":true,"options":[{"value":"hamster","label":"Hamster"},{"value":"kitten","label":"Kitten"},{"value":"puppy","label":"Puppy"},{"value":"bird","label":"Bird"}],"default":"hamster"}]'::jsonb,
     'nature', 5, true, 234);
  END IF;
END $$;
