...
BEGIN
  ...
  WITH transaction_sums AS (
    SELECT 
      ct.user_id,
      SUM(ct.amount)::INTEGER as calculated_balance
  ...
  SELECT 
    ...
    COALESCE(ts.calculated_balance + v_default_credits, v_default_credits)::INTEGER as calculated_balance,
    (p.credits - COALESCE(ts.calculated_balance + v_default_credits, v_default_credits))::INTEGER as difference
