...
BEGIN
  ...
  WHERE 
    -- 排除管理员用户的订单
    (p.role IS NULL OR p.role NOT IN ('admin', 'super_admin'))
    AND (status_filter IS NULL OR py.status = status_filter)
...
-- 2. 更新获取订单总数函数，排除管理员
DROP FUNCTION IF EXISTS get_orders_count(text, text, text, timestamp with time zone, timestamp with time zone);
...
-- 3. 更新获取订单统计概览函数，排除管理员
DROP FUNCTION IF EXISTS get_orders_summary();
