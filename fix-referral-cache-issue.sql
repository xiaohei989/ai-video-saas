-- 修复 accept_referral_code 函数的缓存问题
-- 在积分更新后主动清理用户缓存，确保前端显示最新数据
...
BEGIN
  ...
  PERFORM pg_notify('cache_invalidation', jsonb_build_object(
    ...
    'user_ids', jsonb_build_array(v_inviter_id, invitee_id),
  ...
  RETURN jsonb_build_object(
    ...
    'invitee_credits_after', v_invitee_credits_before + v_reward_credits,
    'inviter_credits_after', v_inviter_credits_before + v_reward_credits,
    'cache_invalidation_sent', true,
    'message', '邀请码处理成功'
  );
