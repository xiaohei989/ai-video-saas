...
-- 1. 重新创建 get_admin_dashboard_stats 函数，排除管理员用户
...
BEGIN
  ...
  WITH user_stats AS (
    SELECT
      ...
      -- 被封用户（排除管理员）
      COUNT(CASE WHEN p.ban_status = 'banned' AND (p.role IS NULL OR p.role NOT IN ('admin', 'super_admin')) THEN 1 END) AS banned_users
    FROM profiles p
  ),
  revenue_stats AS (
    SELECT
...
-- 2. 修改 get_sales_trends 函数，排除管理员支付
...
BEGIN
  RETURN QUERY
  SELECT 
    pm.created_at::date as payment_date,
    SUM(pm.amount_total / 100.0) as daily_revenue
  FROM payment_records pm
  LEFT JOIN profiles p ON p.id = pm.user_id
  WHERE pm.status = 'succeeded'
    AND pm.created_at >= (CURRENT_DATE - INTERVAL '1 day' * days_back)
...
-- 3. 修改 get_subscription_distribution 函数，排除管理员订阅
