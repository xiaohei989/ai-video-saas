-- ========================================
-- 智能延迟缩略图触发器
-- 通过 Edge Function 参数传递延迟信息,避免阻塞数据库
-- ========================================
  ...
  IF NEW.migration_status = 'completed'
    ...
    BEGIN
      ...
      SELECT net.http_post(
        ...
        timeout_milliseconds := 150000  -- 2.5 分钟（包含延迟 + 重试）
      ) INTO response_id;
...
COMMENT ON FUNCTION trigger_auto_generate_thumbnail() IS '迁移完成后智能触发缩略图生成（Edge Function 内部处理延迟）';
...
BEGIN
  ...
  RAISE NOTICE '========================================';
  RAISE NOTICE '✅ 智能延迟: Edge Function 根据迁移时间决定';
  RAISE NOTICE '✅ 不阻塞数据库连接';
  RAISE NOTICE '✅ 超时 150 秒（足够延迟 + 重试）';
  RAISE NOTICE ' ';
  RAISE NOTICE '⏱️  预期时间线:';
  RAISE NOTICE '1. 迁移完成 → migration_status = completed';
  RAISE NOTICE '2. 触发器立即调用 Edge Function';
  RAISE NOTICE '3. Edge Function 检查时间，如需要则延迟 30s';
  RAISE NOTICE '4. 生成缩略图（重试: 0s → 30s → 120s）';
  RAISE NOTICE '5. 总耗时: 30-150 秒';
  RAISE NOTICE '========================================';
END $$;
