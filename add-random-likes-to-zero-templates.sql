...
BEGIN
    ...
    -- 为每个0点赞的模板添加随机点赞
    FOREACH template_record IN ARRAY templates_with_zero_likes
    LOOP
        -- 生成1-1000之间的随机点赞数
        like_count := floor(random() * 1000) + 1;
        
        RAISE NOTICE '📝 为模板 % 添加 % 个点赞', template_record, like_count;
    ...
    FOR template_record IN 
        ...
        WHERE t.id = ANY(templates_with_zero_likes::uuid[])
