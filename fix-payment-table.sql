...
BEGIN
  ...
  revenue_stats AS (
    SELECT
      -- 总收入（排除管理员支付）
      COALESCE(SUM(CASE WHEN p.role IS NULL OR p.role NOT IN ('admin', 'super_admin') THEN py.amount / 100.0 ELSE 0 END), 0) AS total_revenue,
      -- 今日收入（排除管理员支付）
      COALESCE(SUM(CASE WHEN py.created_at >= CURRENT_DATE AND (p.role IS NULL OR p.role NOT IN ('admin', 'super_admin')) THEN py.amount / 100.0 ELSE 0 END), 0) AS revenue_today,
      -- 本周收入（排除管理员支付）
      COALESCE(SUM(CASE WHEN py.created_at >= date_trunc('week', CURRENT_DATE) AND (p.role IS NULL OR p.role NOT IN ('admin', 'super_admin')) THEN py.amount / 100.0 ELSE 0 END), 0) AS revenue_this_week,
      -- 本月收入（排除管理员支付）
      COALESCE(SUM(CASE WHEN py.created_at >= date_trunc('month', CURRENT_DATE) AND (p.role IS NULL OR p.role NOT IN ('admin', 'super_admin')) THEN py.amount / 100.0 ELSE 0 END), 0) AS revenue_this_month
    FROM payments py
...
BEGIN
  ...
  SELECT 
    ...
    SUM(py.amount / 100.0) as daily_revenue
  FROM payments py
  LEFT JOIN profiles p ON p.id = py.user_id
  WHERE py.status = 'completed'
